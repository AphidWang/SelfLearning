...
import forms from '../../lib/ai/config/forms.json';
...
export class MindMapService {
  ...
  getGoal(goalId: string): Goal | undefined {
    return this.goalStore.goals.find(g => g.id === goalId);
  }

  updateGoal(goal: Goal) {
    this.goalStore.updateGoal(goal);
  }

  addStep(goalId: string, step: Step) {
  ...
  updateStep(goalId: string, stepId: string, updates: Partial<Step>) {
    const goal = this.getGoal(goalId);
    if (!goal) return;
  ...
  addTask(goalId: string, stepId: string, task: Task) {
    const goal = this.getGoal(goalId);
    if (!goal) return;
  ...
  updateTask(goalId: string, stepId: string, taskId: string, updates: Partial<Task>) {
    const goal = this.getGoal(goalId);
    if (!goal) return;
