...
// Token 健康檢查中間件
const tokenHealthCheck = async (req: express.Request, res: express.Response, next: express.NextFunction) => {
  const user = (req as any).user;
...
router.get('/token-status', authenticateSupabaseToken, async (req, res) => {
  try {
    ...
    res.json({
      ...
      needsRefresh: timeToExpiry < 15 * 60, // 提前 15 分鐘建議刷新
      user: {
        id: user.id,
        email: user.email
      }
...
export { authenticateSupabaseToken, requireAdmin, tokenHealthCheck };
